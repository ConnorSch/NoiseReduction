...
% iterate through the first 20 realizations to average the signal to find the
% central frequency
% iso surface to create visualization to see the averaging reach the central
% the correct value
...
for i = 1:20
    ...
    locations(i,:) = [X(r2(i),c2(i),p2(i)), Y(r2(i),c2(i),p2(i)), Z(r2(i),c2(i),p2(i))];
end

figure (3) 
plot3(locations(:,1),locations(:,2),locations(:,3),'-o','Color','b','MarkerSize',10,'MarkerFaceColor','#D9FFFF')
hold on
plot3(locations(end,1),locations(end,2),locations(end,3),'-o','Color','r','MarkerSize',10,'MarkerFaceColor','#FFD9D9')
